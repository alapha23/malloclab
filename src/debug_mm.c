...
#if EXPLICIT_LIST == 1 && BIN_TREE == 1
static void delete_node(char *brk)
{
  node_cnt--;
  if((*(size_t *)(brk) == (void *)-1) && (*(size_t *)(brk+WSIZE) == (void *)-1))
  {
  // we delete root
    freeptr == NULL;
    return ;
  ...
  if(*(size_t *)(brk) == (void *)-1)
  {
  // if left end is -1 while right end is not    
      char * next_addr = *((size_t *)(brk+WSIZE));
      PUT(next_addr, (void *)-1);          
  }  
  else if(*(size_t *)(brk+WSIZE) == (void *)-1)
  {
  // if right end is -1 while end end is not    
  }
  else
  {
  // in the middle of the tree, it is not a leaf
      if(*(size_t *)(brk+WSIZE) == (void*)-1)
      {
      // if the end of list, namely freeptr         
        freeptr = *(size_t *)(brk);
...
static void add_node(char * brk)
{
  ...
  char * parent = freeptr; // which is also the old node
  char * new_node = brk;
  // if new node size is larger than freeptr
  // we check the right hand node
  // until it becomes NULL, we add it as next node
  // if it is smaller than freeptr
  while((*(size_t *)(new_node) != (void*)-1 )&&(*(size_t *)(new_node+WSIZE) != (void*)-1))
  {
    if(GET_SIZE(HDRP(brk)) < GET_SIZE(HDRP(*(size_t *)(freeptr))))
    {
    // new node is smaller
      parent = *(size_t *)parent;      
    }
    else
    {
      parent = *(size_t *)(parent+WSIZE);
    }
  }
  // add as next node
  if(*(size_t*)(parent+WSIZE) == (void*)-1)
  {
    PUT(new_node, parent);
    PUT(new_node+WSIZE, (void*)-1);
    PUT(parent+WSIZE, new_node);
  }
  else
  {
  // add as prev node
    PUT(new_node+WSIZE, parent);
    PUT(new_node, (void*)-1);
    PUT(parent, new_node);
  }
}
